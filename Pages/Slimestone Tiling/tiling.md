...
### RedenMC
Существует китайский сайт, представляющий из себя архив слаймстоун механизмов: [RedenMC](https://redenmc.com/litematica). Он примечателен тем, что многие посты имеют опцию выставить размер постройки. Сайт автоматически генерирует лайтматику нужного Вам размера.

### Python
Игроки, у которых установлен язык **[Python](https://www.python.org/downloads/)** версии 3.9 или выше, могут генерировать нужные лайтматики просто по щелчку мыши прямо у себя на компьютере.

*Скрипт написан третьими лицами и не аффилирован с авторами данной Вики.

Незер World Eater'ы, сталкивающие древние обломки к краям периметра, часто называют **SSNWE**, сокращённо от *Side Sweep Nether World Eater*. Конкретно для низкого Chronos-SSNWE игрок \_spindle\_ сделал скрипт для генерации лайтматик с нужными размерами. Дополнительно он вычисляет размеры траншей для обломков и размечает их листвой.

Ссылка на скрипт: **[zip-файл](media/sidesweep_14h_tiling.zip)**

**Установка** (если уже есть Python)
- Установите библиотеку `litemapy`, прописав в консоли `pip install litemapy`
- Далее `pip install scipy`
- Скачайте `.zip` файл и разархивируйте его в папку `.minecraft/schematics`
- Чтобы запустить скрипт, можно дважды кликнуть по файлу `main.py` в папке `.minecraft/schematics/sidesweep_14h_tiling`
- В открывшемся окне введите ширину и длину через пробел, например `528 528`
- Лайтматика появится сразу в папках схематик и будет доступна в игре
...
**[14h SSNWE Script](media/sidesweep_14h_tiling.zip)**
